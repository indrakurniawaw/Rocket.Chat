...
(function (w) {
	...
	var validCallbacks = ['chat-maximized', 'chat-minimized', 'chat-started', 'chat-ended', 'pre-chat-form-submit', 'offline-form-submit'];
	...
	var init = function (url) {
		...
		chatWidget.innerHTML =
			...
			'<iframe id="rocketchat-iframe" src="' +
			url +
			'" style="width:100%;height:100%;border:none;background-color:transparent" allowTransparency="true" allow="microphone; camera"></iframe> ' +
	...
	if (typeof w.KatalisApp.url !== 'undefined') {
		init(w.KatalisApp.url);
	}

	var queue = w.KatalisApp._;

	w.KatalisApp = w.KatalisApp._.push = function (c) {
		c.call(w.KatalisApp.livechat);
	};

	// exports
	w.KatalisApp.livechat = {
		// methods
